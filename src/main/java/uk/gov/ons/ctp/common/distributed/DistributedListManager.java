...
/**
 * Genericized Interface for apps to code against for distributed lists of
 * lists. This manager allows for the distributed storage/retrieval of arbitrary
...
public interface DistributedListManager<T> {
  ...
  /**
   * Store the list against the given key in the distributed store NOTE: this
   * thread must have locked the list container either directly or thru the
   * findList()
   *
  ...
  /**
   * get the list associated with the given key that was stored by any
   * application instance ie the list returned is the flattened, 'super' list of
   * lists for key so if 3 instances of the same app, using an identically
   * configured list manager, store individually :
   *
   * instance 1 store "keyA" : [1,2,3] instance 2 store "keyA" : [4,5,6]
