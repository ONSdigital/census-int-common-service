...
public class CustomDateSerialiser extends JsonSerializer<Date> {
  ...
  private SimpleDateFormat dateFormatter = new SimpleDateFormat(DateTimeUtil.DATE_FORMAT_IN_JSON);

  @Override
  public void serialize(Date value, JsonGenerator jsonGenerator, SerializerProvider provider)
      throws IOException, JsonProcessingException {
    // Note that all threads are using the same formatter, which needs synchronisation as it is not thread safe.
