...
public class RestClientErrorHandler implements ResponseErrorHandler {
  ...
  /**
   * Actually deal with error - here we just log it and allow the RestClient to handle the error -
   * because RestClient needs to handle the deserialization of both the normal response type OR the
   * error type
   *
   * @param response the response
   * @throws IOException if we cannot delve into the response
